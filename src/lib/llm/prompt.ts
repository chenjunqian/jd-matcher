import type { UserMatchedJobPromptInput } from "../types.js";

const PROMPT_TEMPLATE = `"You are an expert career advisor and AI-powered job matching assistant. Your task is to analyze a given resume and match it with the most suitable jobs from a provided list of job descriptions.
...
export function getJobMatchPromptTemplate(): string {
  return PROMPT_TEMPLATE;
}

export function generateResumeMatchPrompt(promptTemp: string, resume: string, expectation: string, jobList: string): string {
  return promptTemp.replace("{{ resume }}", resume).replace("{{ expectations }}", expectation).replace("{{ job_list }}", jobList);
}

export function serializeJobListToJson(jobs: UserMatchedJobPromptInput[]): string {
